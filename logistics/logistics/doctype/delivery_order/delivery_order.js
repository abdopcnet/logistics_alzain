// 1. Delivery Order: Main logic (validation, protection, auto-JE)
frappe.ui.form.on('Delivery Order', {
        ...
        if (!frm.is_new()) {
            frappe.call({
                ...
                callback: function(response) {
                    ...
                    let old_fees = (old_doc && old_doc.table_uuzw) ? old_doc.table_uuzw : [];
                    let new_fees = frm.doc.table_uuzw || [];
    ...
    after_save: function(frm) {
        ...
        function create_journal_entries_sequentially(index) {
            if (!frm.doc.table_uuzw || index >= frm.doc.table_uuzw.length) return;
            let row = frm.doc.table_uuzw[index];
    ...
    transaction_no: function(frm) {
        if (frm.doc.transaction_no) {
            // 1.3.1 Clear and fetch fcl_list
            ...
            // 1.3.2 Clear and fetch cbm_list
            ...
            frappe.model.with_doc('Transaction', frm.doc.transaction_no, function () {
                ...
                for (const source_row of source_doc.cbm_list) {
                    ...
                    target_row.gross_wt = source_row.gross_wt;
    ...
    refresh(frm) {
        ...
        frm.add_custom_button('🔗 Fetch Journal Entry', function() {
            if (!frm.doc.transaction_no || !frm.doc.customer) {
            ...
            frappe.call({
                ...
                args: {
                    ...
                    filters: {
                        ...
                        custom_delivery_order: frm.doc.name,
...
function create_journal_entry(accounts, journal_entry_title, row_idx, frm, posting_date, callback) {
    let row = frm.doc.table_uuzw.find(r => r.idx === row_idx);
    frappe.call({
                ...
                frappe.call({
                    ...
                    callback: function(submit_response) {
                        if (submit_response.message) {
                            // Save the doc after linking Journal Entry
...
// 3. Agent Fee child table: protect delete, auto-fill customer/bol_no
frappe.ui.form.on('Agent Fee', {
    // 3.1 Auto-fill customer and bol_no on add row
    table_uuzw_add: function(frm, cdt, cdn) {
